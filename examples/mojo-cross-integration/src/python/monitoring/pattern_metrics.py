...
class PatternMetrics:
    ...
    def get_script_distribution(self) -> Dict[str, float]:
        ...
        return {script: count / total for script, count in self.script_counts.items()}
    ...
    def get_confidence_trends(self) -> Dict[str, float]:
        ...
        recent_window = 10
        recent_confidence = (
            confidence[-recent_window:]
            if len(confidence) > recent_window
            else confidence
        )
    ...
    def get_time_based_metrics(
        ...
        recent_indices = [
            i for i, ts in enumerate(self.timestamps) if ts >= window_start
        ]
