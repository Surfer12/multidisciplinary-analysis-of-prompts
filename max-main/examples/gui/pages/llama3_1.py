...
def messages_to_llama3_prompt(messages: list[dict[str, str]]) -> str:
    ...
    for message in messages:
        prompt_string += f"<|start_header_id|>{message['role']}<|end_header_id|>\n\n"
...
max_length = st.sidebar.number_input("Max input and output tokens", 0, 128_000, 12_000)
...
if prompt := st.chat_input("Send a message to llama3", disabled=disable_chat):
    ...
    with st.chat_message("assistant", avatar="🦙"):
        response = asyncio.run(stream_output(st.session_state["model"], prompt_string))
