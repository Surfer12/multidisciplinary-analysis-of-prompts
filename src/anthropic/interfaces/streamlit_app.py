    ...
    with st.sidebar:
        ...
        if st.session_state.meta_cognitive_state["adaptations"]:
            with st.expander("Tool Adaptations", expanded=True):
                for adaptation in st.session_state.meta_cognitive_state["adaptations"][
                    -5:
                ]:
        ...
        if st.session_state.meta_cognitive_state["metrics"]:
            with st.expander("Performance Metrics", expanded=True):
                ...
                with col1:
                    st.metric(
                        "Stability", f"{metrics.get('execution_stability', 0):.2f}"
                    )
                with col2:
...
def update_meta_cognitive_state(tool_result: ToolResult):
    ...
    if hasattr(tool_result, "adaptations"):
        st.session_state.meta_cognitive_state["adaptations"].extend(
            tool_result.adaptations
        )
    ...
    st.session_state.tool_history.append(
        {"timestamp": datetime.now().isoformat(), "tool_result": tool_result}
    )
...
async def main():
    ...
    chat, http_logs, insights = st.tabs(
        ["Chat", "HTTP Exchange Logs", "Meta-Cognitive Insights"]
    )
